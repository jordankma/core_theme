...
class ContestSeasonController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        if(!empty($season)){
            $start_date = date_create_from_format('Y-m-d H:i:s', $season->start_date);
            $end_date = date_create_from_format('Y-m-d H:i:s', $season->end_date);
            $season->start_date = $start_date->format('d-m-Y H:i');
            $season->end_date = $end_date->format('d-m-Y H:i');
        }
        $data = [
            'season' => $season,
            'environment' => $this->env->getEnvironment(),

...
{{-- Page title --}}
@section('title'){{ $title = trans('contest-contestmanage::language.titles.contest_topic.update') }}@stop
        ...
        <div class="row">
            <div class="panel panel-primary" id="hidepanel1">
                ...
                <div class="panel-body ">
                    <!--main content-->
                    {!! Form::model($topic, ['url' => route('contest.contestmanage.contest_topic.update',['topic_id' => $topic->topic_id]), 'method' => 'put', 'class' => 'bf', 'files'=> true]) !!}
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group {{ $errors->first('name', 'has-error') }}">
                                {!! Form::text('name', $topic->display_name, array('class' => 'form-control', 'autofocus'=>'autofocus','placeholder'=> trans('contest-contestmanage::language.placeholder.contest_topic.name_here'))) !!}
                                <span class="help-block">{{ $errors->first('name', ':message') }}</span>
                            </div>
                            <label>Vòng thi (*)</label>
                            <div class="form-group {{ $errors->first('round', 'has-error') }}">
                                {!! Form::text('round', !empty($topic->round->display_name)?base64_decode($topic->round->display_name):null, array('class' => 'form-control round', 'autofocus'=>'autofocus', 'readOnly' => 'readOnly', 'placeholder'=> trans('contest-contestmanage::language.placeholder.contest_topic.round'))) !!}
                            ...
                            <div class="form-group">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group {{ $errors->first('number', 'has-error') }}">
                                        {!! Form::number('number', $topic->order, array('class' => 'form-control')) !!}
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group {{ $errors->first('type', 'has-error') }}">
                                        {!! Form::select('type', $type,$topic->topic_type, array('class' => 'form-control')) !!}
                                ...
                                <div class="panel-body">
                                    ...
                                    @if(!empty($target))
                                        @foreach($target as $key => $item)
                                            ...
                                            <div class="form-group">
                                                {!! Form::text('question_pack[]', $question_pack_name[$key], array('class' => 'form-control question_pack','id' => $key, 'autofocus'=>'autofocus', 'readOnly' => 'readOnly')) !!}
                                            </div>
                                            {!! Form::hidden('question_pack_id['.$key.']', $question_pack_id[$key], array('class' => 'question_pack_id','id' => $key.'_id', 'readOnly' => 'readOnly')) !!}
                                        @endforeach
                                    @else
                            ...
                            <div class="form-group" id="config_container">
                                @if(!empty($topic_config))
                                    @php
                                        $idx = 1;
                                    @endphp
                                    @foreach($topic_config as $key => $value)
                                        @php
                            ...
                            <div class="form-group {{ $errors->first('description', 'has-error') }}">
                                {!! Form::textarea('description', base64_decode($topic->description), array('class' => 'form-control', 'autofocus'=>'autofocus','placeholder'=> trans('contest-contestmanage::language.placeholder.contest_topic.description'))) !!}
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="panel panel-primary">
                                <div class="panel-body">
                                    ...
                                    <div class="form-group {{ $errors->first('topic_exam_repeat_condition', 'has-error') }}">
                                        {!! Form::number('topic_exam_repeat_condition', null, array('class' => 'form-control', 'min' => 0)) !!}
                                    ...
                                    <div class="form-group {{ $errors->first('topic_point_condition', 'has-error') }}">
                                        {!! Form::number('topic_point_condition', null, array('class' => 'form-control', 'min' => 0)) !!}
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="start_date" value="{{ $topic->start_date }}" class="form-control" id="start_date"/>
                            </div>
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="end_date" value="{{ $topic->end_date }}" class="form-control" id="end_date"/>
                            </div>

                            <label>Thông báo sau ngày kết thúc (*)</label>
                            <div class="form-group {{ $errors->first('end_notify', 'has-error') }}">
                                {!! Form::textarea('end_notify',  base64_decode($topic->end_notify), array('class' => 'form-control', 'autofocus'=>'autofocus','placeholder'=> trans('contest-contestmanage::language.placeholder.contest_topic.after_end_notify'))) !!}
                    ...
                    <div class="row">
                        <div class="panel panel-primary">
                            <div class="panel-body">
                                ...
                                <div id="topic_round">
                                    @if(!empty($topic->topic_round))
                                        @php
                                            $topic_round = json_decode($topic->topic_round);
                                        @endphp
                                        @foreach($topic_round as $key=>$value)
                                            <div class="col-md-6">
                                                <label>Thể lệ vòng {{ !empty($value->round_name)?$value->round_name:'' }}</label>
                                                <div class="form-group">
                                                    <input type="hidden" name="topic_round[{{ $key }}][round_name]" value="{{ !empty($value->round_name)?$value->round_name:'' }}">
                                                    <textarea name="topic_round[{{ $key }}][rule_text]" class="form-control" id="ckeditor_full{{ $key }}">{{ !empty($value->rule_text)?base64_decode($value->rule_text):'' }}</textarea></div>
                                            </div>
                                        @endforeach
                                    @endif
                                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group {{ $errors->first('rules', 'has-error') }}">
                            {!! Form::textarea('rules', $topic->rule, array('class' => 'form-control', 'id' => 'ckeditor_full','placeholder'=> trans('contest-contestmanage::language.placeholder.contest_topic.rules'))) !!}
...
@section('footer_scripts')
    ...
    {{--@if(!empty($topic->topic_round))--}}
        ...
        {{--@foreach($topic_round as $key=>$value)--}}
            {{--<script>--}}
                {{--CKEDITOR.replace( 'ckeditor_full{{ $key }}');--}}
            {{--</script>--}}
                ...
                if (res && res.count > 0) {
                    $.each(res.list_round, function (key, item) {
                        ...
                        var html = '<div class="col-md-6">' +
                            ...
                            '<div class="form-group"><textarea name="topic_round['+key+'][rule_text]" class="form-control" id="' +id+ '"></div> ' +
                            '</div>';

            ...
            <div class="panel panel-primary ">
                <div class="panel-heading clearfix">
                    ...
                    <div class="pull-right">
                        <a href="{{ route('vne.companionunit.create') }}" class="btn btn-sm btn-default"><span
                                    class="glyphicon glyphicon-plus"></span> {{ trans('vne-companionunit::language.buttons.create') }}</a>
                    </div>
                </div>
                <br/>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="table">
                            ...
                            <tr class="filters">
                                ...
                                <th style="width: 150px">Nhóm đơn vị</th>
                                <th style="width: 150px">Tên đơn vị</th>
                                <th style="width: 120px">{{ trans('vne-companionunit::language.table.comlink') }}</th>
                                <th style="width: 100px">{{ trans('vne-companionunit::language.table.comnote') }}</th>
                                <th style="width: 120px">{{ trans('vne-companionunit::language.table.img') }}</th>
                                <th style="width: 100px">{{ trans('vne-companionunit::language.table.created_at') }}</th>
                                <th style="width: 100px">{{ trans('vne-companionunit::language.table.updated_at') }}</th>
                                <th>{{ trans('vne-companionunit::language.table.action') }}</th>
                            </tr>
                            </thead>
                        </table>
                    </div>
        ...
        $(document).ready(
            function () {
                var table = $('#table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: '{{ route('vne.companionunit.data') }}',
                    columns: [
                        { data: 'DT_Row_Index', name: 'id' },
                        { data: 'comtype', name: 'comtype' },
                        { data: 'comname', name: 'comname' },
                        { data: 'comlink', name: 'comlink' },
                        { data: 'comnote', name: 'comnote' },
                        { data: 'img', name: 'img' },

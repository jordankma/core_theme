...
{{-- Page title --}}
@section('title'){{ $title = trans('vne-newsrldv::language.titles.news.edit') }} @stop
{{-- page styles --}}
    ...
    <!--main content-->
        <div class="row">
            <div class="the-box no-border">
                <form role="form" action="{{route('vne.newsrldv.news.update',$news->news_id)}}" method="post" enctype="multipart/form-data" id="form-edit-news">
                    ...
                    <div class="row">
                        <div class="col-md-8 col-sm-8">
                            ...
                            <div class="form-group">
                                <label>{{trans('vne-newsrldv::language.form.text.title')}}<span style="color: red">(*)</span></label>
                                <input type="text" required name="title" value="{{$news->title}}" class="form-control" placeholder="{{trans('vne-newsrldv::language.form.title_placeholder')}}">
                            </div>
                            <div class="form-group">
                                <label>{{trans('vne-newsrldv::language.form.text.desc')}}</label><br>
                                <textarea rows="5" cols="101" name="desc"  class="form-control" placeholder="{{trans('vne-newsrldv::language.form.desc_placeholder')}}">{{$news->desc}}</textarea>
                            </div>
                            {{-- <div class="form-group" style="display: none;visibility: hidden;">
                                <label for="news-text">
                                    <input type="radio" id="news-text" name="type" value="1" @if($news->type==1) checked="checked" @endif class="type_news">
                                    {{trans('vne-newsrldv::language.form.text.news_text')}}
                                </label> 
                                <label for="news-image">
                                    ...
                                    {{trans('vne-newsrldv::language.form.text.news_image')}}
                                </label>
                            </div> --}}
                            <div class="area-new-text" {{-- @if($news->type==1) style="display: block;" @else style="display: none;" @endif --}}>
                                <div class="form-group" >
                                    <label>{{trans('vne-newsrldv::language.form.text.content')}} </label><br>
                                    <div class='box-body pad form-group'>
                                        <textarea name="content" id="ckeditor" placeholder="{{trans('vne-newsrldv::language.form.content_placeholder')}}">{{$news->content}}</textarea>
                                    </div>
                                </div>
                            </div>
                            {{-- <div class="area-new-image" @if($news->type==2) style="display: block;" @else style="display: none;" @endif>
                                <div class="form-group" id="list-item" >
                                    <a id="lfm1" data-input="thumbnail1" data-preview="holder1" class="btn btn-primary">
                                        <i class="fa fa-picture-o"></i> {{trans('vne-newsrldv::language.label.choise_image')}}
                                    </a>
                                    <div class="table-responsive col-md-10" >
                                        <table class="grid table table-bordered table-sortable" style='font-size: 12px;'>
                                            <thead>
                                            <th>{{trans('vne-newsrldv::language.table.news.image')}}</th>
                                            <th>{{trans('vne-newsrldv::language.table.news.url')}}</th>
                                            <th>{{trans('vne-newsrldv::language.table.news.actions')}}</th>
                                            </thead>
                                            <tbody id="list-file">
                        ...
                        <div class="col-md-4 col-sm-4">
                            <div class="form-group">
                                <label>{{trans('vne-newsrldv::language.form.text.box')}} <span style="color: red">(*)</span></label>
                            ...
                            <div class="form-group">
                                <label>{{trans('vne-newsrldv::language.form.text.cat')}} </label><br>
                            ...
                            <div class="form-group area-tag">
                                ...
                                <div class="input-group">
                                    <input type="text" name="news_tag_add" value="" class="form-control" placeholder="{{trans('vne-newsrldv::language.form.tags_placeholder')}}" id="text-tag">
                            ...
                            <label>{{trans('vne-newsrldv::language.form.text.image')}}</label>
                            <div class="input-group">
                               <span class="input-group-btn">
                            ...
                            <div class="form-group">
                                ...
                                <label for="hot">{{trans('vne-newsrldv::language.form.text.news_hot')}}    </label>
                                <input type="radio" @if($news->is_hot==2) checked @endif id="normal" name="is_hot" value="2">
                                <label for="normal">    {{trans('vne-newsrldv::language.form.text.news_normal')}}</label>
                            </div>
                            <div class="form-group">
                                <label>{{trans('vne-newsrldv::language.form.text.priority')}}</label>
                                <input class="form-control" type="number" name="priority" value="{{$news->priority}}" placeholder="{{trans('vne-newsrldv::language.form.priority_placeholder')}}">
                            </div>
                            <div class="form-group">
                                <label>{{trans('vne-newsrldv::language.form.text.key_seo')}}</label> <br>
                                <input type="text" name="key_word_seo[]" value="{{$list_key_word_seo_string}}" class="form-control" data-role="tagsinput" placeholder="{{trans('vne-newsrldv::language.form.seo_key_word_placeholder')}}">
                            </div>
                            <div class="form-group">
                                <label>{{trans('vne-newsrldv::language.form.text.desc_seo')}}</label>
                                <textarea rows="5" cols="101" name="desc_seo" class="form-control" placeholder="{{trans('vne-newsrldv::language.form.desc_seo_placeholder')}}">{{$news->desc_seo}}</textarea>
                            </div>
                        </div>
                        <div class="form-group text-center">
                            <button type="submit" class="btn btn-success">{{trans('vne-newsrldv::language.buttons.update')}}</button>
                            <a href="" class="btn btn-danger">{{trans('vne-newsrldv::language.buttons.discard')}}</a>
                        </div>
                        <!-- /.col-sm-8 -->
                    </div>
                    <!-- /.row -->
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('body').on('click','#add-tag',function(e){
                ...
                if(list_tag!=''){
                    $.ajax({
                        url: "{{route('vne.newsrldv.tag.ajax.add')}}",
                        headers: {

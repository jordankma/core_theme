...
use Dhcd\News\App\Http\Requests\NewsTagRequest;
...
{	
	...
	public function manager(){
		return view('DHCD-NEWS::modules.news.news_tag.manager');
	}
	/**
	...
	public function create(){
		return view('DHCD-NEWS::modules.news.news_tag.create');
	}
	/**
	...
	public function add(NewsTagRequest $request){
		...
		if ($news_tag->news_tag_id) {
            ...
            return redirect()->route('dhcd.news.tag.manager')->with('success', trans('dhcd-news::language.messages.success.create'));
        } else {
            return redirect()->route('dhcd.news.tag.manager')->with('error', trans('dhcd-news::language.messages.error.create'));
        }
	}
    /**
	...
	public function show(NewsTagRequest $request){
		...
		return view('DHCD-NEWS::modules.news.news_tag.edit',compact('news_tag'));
	}	
	public function update(NewsTagRequest $request){
		...
		if ($news_tag->save()) {
            ...
            return redirect()->route('dhcd.news.tag.manager')->with('success', trans('DHCD-NEWS::language.messages.success.update'));
        } else {
            return redirect()->route('dhcd.news.tag.manager')->with('error', trans('DHCD-NEWS::language.messages.error.update'));
        }
	}
    public function delete(NewsTagRequest $request)
    {
        ...
        if (null != $news_tag) {
            ...
            DB::table('dhcd_news_has_tag')->where('news_tag_id',$news_tag_id)->delete();
            activity('news_tag')
                ->performedOn($news_tag)
                ->withProperties($request->all())
                ->log('User: :causer.email - Delete News tag - news_tag_id: :properties.news_tag_id, name: ' . $news_tag->name);
            return redirect()->route('dhcd.news.tag.manager')->with('success', trans('DHCD-NEWS::language.messages.success.delete'));
        } else {
            return redirect()->route('dhcd.news.tag.manager')->with('error', trans('DHCD-NEWS::language.messages.error.delete'));
        }
    }
    public function getModalDelete(Request $request)
    {
        ...
        if (!$validator->fails()) {
            try {
                $confirm_route = route('dhcd.news.tag.delete', ['news_tag_id' => $request->news_tag_id]);
    ...
    public function data()
    {
        ...
        return Datatables::of($list_news_tag)
            ...
            ->addColumn('actions', function ($list_news_tag) {
                ...
                if ($this->user->canAccess('dhcd.news.tag.confirm-delete')) {
                    $actions .='<a href=' . route('dhcd.news.tag.confirm-delete', ['news_tag_id' => $list_news_tag->news_tag_id]) . ' data-toggle="modal" data-target="#delete_confirm"><i class="livicon" data-name="trash" data-size="18" data-loop="true" data-c="#f56954" data-hc="#f56954" title="delete news tag"></i></a>';
                }
                return $actions;
            })
            ->rawColumns(['actions'])

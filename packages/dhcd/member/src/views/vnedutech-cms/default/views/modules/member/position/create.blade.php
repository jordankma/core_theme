...
{{-- Page title --}}
@section('title'){{ $title = trans('dhcd-member::language.titles.position.create') }}@stop
    ...
    <section class="content paddingleft_right15">
        ...
        <div class="row">
            <div class="the-box no-border">
                <!-- errors -->
                <form action="{{ route('dhcd.member.position.add') }}" method="post" id="form-add-position">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}"/>
                    <div class="row">
                        <div class="col-sm-4">
                            <label>{{ trans('dhcd-member::language.form.title_position.name') }}</label>
                            <div class="form-group">
                                <input type="text" name="name"  class="form-control" placeholder="{{ trans('dhcd-member::language.placeholder.position.name') }}">
                            </div>
                        </div>
                        <!-- /.col-sm-8 -->
                        <div class="col-sm-4">

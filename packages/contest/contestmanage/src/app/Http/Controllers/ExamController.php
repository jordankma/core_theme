...
//        }
    ...
    public function getTopCandidate(Request $request)
    {
        ...
        $top = !empty($request->top) ? (((int)$request->top > 50)?50:(int)$request->top) : 10;
        ...
        if (!empty($request->class_id)) {
            $match['class_id'] = (int)$request->class_id;
        }
        if (!empty($request->repeat_time)) {
            $match['repeat_time'] = (int)$request->repeat_time;
        }
        if(!empty($match)){
        ...
        if(!empty($data)){
            $data_arr = [];
            $idx = !empty($skip)?(int)$skip:0;
            foreach ($data as $key => $value){
                if(!empty($value->_id[0])){
                    if(count($data_arr) < ($top)){
                        $idx++;
                        $data_arr[$key] = [];
                        $data_arr[$key][] = $idx;
                        $data_arr[$key][] = $value->_id;
...
//        ];
    ...
    public function getTopResult(Request $request)
    {
        ...
        if (!empty($request->repeat_time)) {
            $cond['repeat_time'] = (int)$request->repeat_time;
        }
         if (!empty($request->class_id)) {
            $cond['class_id'] = (int)$request->class_id;
        }

        $result = ContestResult::where($cond)->orderBy('total_point','desc')->orderBy('used_time', 'asc')->paginate($top);
    ...
    public function removeTimestamp(){
        $data = ContestResult::where('updated_at','exists',true)->skip(0)->take(100)->get();
        if($data->count() >0){
            foreach ($data as $key => $value){
                $value->unset('updated_at');
                try {
                    $value->update();
                    echo '<pre>';print_r($value->_id);echo '</pre>';
                }
                catch (\Exception $e){
    ...
    public function syncUserInfo(Request $request){
        $limit = !empty($request->limit) ? (int)$request->limit : 1000;
        $data = ContestResult::where('name','exists',false)->skip(0)->take($limit)->get();
        if (!empty($data)) {
            foreach ($data as $key => $value){
                $user_info = UserContestInfo::where('member_id',$value->member_id)->first();
                if(!empty($user_info)){
                    foreach ($user_info->getAttributes() as $key1 => $value1){
                        if(!in_array($key1,array('_id','member_id','created_at','updated_at','deleted_at'))){
                            $value->$key1 = $value1;
                        }
                    }
                    try{
                        if($value->update()){
                            echo '<pre>';print_r($value->_id.' - done');echo '</pre>';
                        }
            ...
            echo '<pre>';print_r('data empty');echo '</pre>';
        }
    }

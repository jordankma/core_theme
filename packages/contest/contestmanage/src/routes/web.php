...
//    API get json load form động, param: type
    ...
    Route::get('sync_userinfo', 'ExamController@syncUserInfo')->name('sync_userinfo');
...
//        });
        ...
        Route::group(['prefix' => 'contest/contestmanage/contest_user', 'as' => 'contest.contestmanage.contest_user.'], function () {
            Route::get('log', 'ContestUserController@log')->name('log');
            Route::get('show', 'ContestUserController@show')->name('show');
            Route::post('change_password', 'ContestUserController@changePassword')->name('change_password');
            Route::post('reset_exam', 'ContestUserController@resetExam')->name('reset_exam');
            Route::get('data', 'ContestUserController@data')->name('data');
            Route::get('manage', 'ContestUserController@manage')->where('as','Quản lý thí sinh')->name('manage');
        });

        Route::group(['prefix' => 'contest/contestmanage/search_account', 'as' => 'contest.contestmanage.search_account.'], function () {
            Route::get('log', 'SearchAccountController@log')->name('log');
            Route::get('data', 'SearchAccountController@data')->name('data');
            Route::get('manage', 'SearchAccountController@manage')->where('as','Danh sách tài khoản tra cứu')->name('manage');
            Route::post('update', 'SearchAccountController@update')->name('update');
            Route::post('add', 'SearchAccountController@add')->name('add');
            Route::get('create', 'SearchAccountController@create')->name('create');
        });
    });
});
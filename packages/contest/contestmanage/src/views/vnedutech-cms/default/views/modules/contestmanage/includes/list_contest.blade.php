            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="table_item">
                    ...
                    <tr class="filters">
                        ...
                        <th>{{ trans('contest-contestmanage::language.table.contest.name') }}</th>
...
<script>
    var table = $('#table_item').DataTable({
        ...
        destroy: true,
        ajax: {
            "url":'http://quiz2.vnedutech.vn/admin/api/contest/list_contest',
            "data":{
                csrf_token:'{{ csrf_token() }}'
            }
        },
        columns: [
            { data: 'contest_id', name: 'contest_id' },
            { data: 'name', name: 'name' },
            { data: 'contest_id', name: 'actions', orderable: false, searchable: false, className: 'fit-content', "render": function (data, type, row, meta) {
                return '<a href="javascript:void(0)" c-data="' +data+  '" d-data="' + row['name'] + '" class="btn btn-success question_choose">Chọn</a>';
            }}
        ]
    });
